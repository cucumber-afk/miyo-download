...
export default function MiYoAnimationCard({ item, onPreview, screenOverride }) {
  ...
  return <article className="miyo-animation-card">
    ...
    <div className="miyo-animation-card-body"><div className="miyo-animation-card-meta"><h2>{item.title}</h2><p className="card-mood">{item.category}</p>{item.publishedAt && <p className="animation-published-at" title={getUtcTimestampTitle(item.publishedAt)}>Published {formatUtcDate(item.publishedAt)}</p>}</div><div className="miyo-animation-card-actions"><button className="preview-button" type="button" onClick={() => onPreview(item)}>Preview</button>{formats.length > 0 && <div className="miyo-animation-download-group"><p className="miyo-animation-download-label">Download Formats</p><div className="miyo-animation-downloads" aria-label="Download Formats">{item.downloads.gif?.src && <a className="miyo-download-link" href={getDownloadUrl(item.downloads.gif, getDownloadFileName(item.downloads.gif, item.title, 'gif'))} download={getDownloadFileName(item.downloads.gif, item.title, 'gif')} aria-label={`Download ${item.title} GIF, no audio`}>GIF · No Audio <Download size={13} /></a>}{item.downloads.mp4?.src && <a className="miyo-download-link" href={getDownloadUrl(item.downloads.mp4, getDownloadFileName(item.downloads.mp4, item.title, 'mp4'))} download={getDownloadFileName(item.downloads.mp4, item.title, 'mp4')} aria-label={`Download ${item.title} MP4`}>MP4 <Download size={13} /></a>}</div></div>}</div></div>
  </article>;
}
